...
# Ajouter la colonne success
print("📝 Ajout de la colonne 'success'...")
df["success"] = df.apply(_compute_success, axis=1)

# Vérifier
print(f"✅ Colonne 'success' créée avec {df['success'].notna().sum()} valeurs")

# Sauvegarder
df.to_csv("data/prediction_dataset_enriched.csv", index=False)
print("✅ Sauvegardé dans data/prediction_dataset_enriched.csv")

# Vérifier la sauvegarde
df_check = pd.read_csv("data/prediction_dataset_enriched.csv")
print(f"✅ Vérification: 'success' présent? {'success' in df_check.columns}")

# Afficher les stats
completed = df[df["success"].notna()]
print(f"✅ Prédictions finalisées: {len(completed)}")
print(f"🎯 Succès: {completed['success'].astype(bool).sum()}")
