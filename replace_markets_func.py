...
start = text.index("def _markets_from_matrix")
    ...
    def _markets_from_matrix(
        ...
        def _baseline() -> Dict[str, float]:
            ...
            base = {
                ...
                "btts_yes_over_2_5": 1.0 if (total_goals >= 3 and current_home > 0 and current_away > 0) else 0.0,
            }
            ...
            for key in ["1/1", "1/N", "1/2", "N/1", "N/N", "N/2", "2/1", "2/N", "2/2"]:
                base[f"htft_{key}"] = 1.0 if key == "1/1" else 0.0
            return base
        ...
        # Halftime probabilities (approx: 45% / 55% split)
        lambda_home = max(0.05, float(matrix[0][0]) ** 0)  # placeholder to access closure scope
        return_dict = dict(aggregates)

        return_dict["ht_home"] = 0.0
        return_dict["ht_draw"] = 0.0
        return_dict["ht_away"] = 0.0
        for key in ["1/1", "1/N", "1/2", "N/1", "N/N", "N/2", "2/1", "2/N", "2/2"]:
            return_dict[f"htft_{key}"] = 0.0
